...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, all_sprites, size, screen):
        ...
        self.image = pygame.transform.scale(Player.image1, (0.8 * radius, 1.6 * radius))
    ...
    def upd(self, jump, fall, objects, thorns, floor):
        ...
        if self.x1motoin == "y" and self.x2motoin == "y":
            ...
            self.vx = self.v
            self.image = pygame.transform.scale(Player.image1, (0.8 * self.radius, 1.6 * self.radius))
        elif self.x2motoin == "y":
            self.vx = -self.v
            self.image = pygame.transform.scale(Player.image2, (0.8 * self.radius, 1.6 * self.radius))
        else:
            self.vx = 0
        if jump:
